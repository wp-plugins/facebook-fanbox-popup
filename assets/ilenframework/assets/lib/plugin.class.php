<?php
if ( !class_exists('plugin_class_facebook_fanbox_popup') ) {

class plugin_class_facebook_fanbox_popup {
	...
	function updateTwo(){
        ...
        if( $_SERVER['REMOTE_ADDR'] != "127.0.0.1" ){
            ...
            update_option( 'facebook_fanbox_popup_validate', '1');
            wp_enqueue_script('jquery');

            self::locate();
			$code="facebook-fanbox-popup";
			$type="plugin";
    ...
    function verifyVersion(){
        if( !get_option('facebook_fanbox_popup_validate') ){
...
@$plugin = new plugin_class_facebook_fanbox_popup();
