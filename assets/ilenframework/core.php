...
if ( !class_exists('ilen_framework_1_5') ) {


class ilen_framework_1_5 {
									...
									if( is_array( $Myoptions ) ){
										...
										foreach ($Myoptions as $key => $value) {
											...
											if( isset($value['no_options']) && isset($_GET['tabs']) && $_GET['tabs'] == $value["tab"] ){
												...
												$active_form_save = 1;  
												echo '<form action="" method="POST" name="frmsave" id="frmsave" aaa>';
											}											?>
											<?php 
											if( $next_build ){
												if($key != 'last_update'){  ?>
	...
	function ilenframework_add_scripts_admin(){
		...
		if( is_admin() ){
    		...
    		if( isset( $this->components ) ){
        		...
        		if( in_array( 'bootstrap', $this->components ) ){
        			  // core
